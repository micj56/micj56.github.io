...
static void disasmR(char *opName, int r1) {
  ...
  if (ip == NULL) {
    ...
    switch (ip->format) {
      case FORMAT_N:
        ...
        break;
      case FORMAT_R:
        disasmR(ip->name, (instr >> 21) & 0x1F);
        break;
      case FORMAT_RH:
        disasmRH(ip->name, (instr >> 21) & 0x1F, instr & 0x0000FFFF);
        break;
      case FORMAT_RHh:
        disasmRHh(ip->name, (instr >> 21) & 0x1F, instr & 0x0000FFFF);
        break;
      case FORMAT_RRH:
        disasmRRH(ip->name, (instr >> 21) & 0x1F,
                  (instr >> 16) & 0x1F, instr & 0x0000FFFF);
        break;
      case FORMAT_RRS:
        disasmRRS(ip->name, (instr >> 21) & 0x1F,
                  (instr >> 16) & 0x1F, instr & 0x0000FFFF);
        break;
      case FORMAT_RRR:
        disasmRRR(ip->name, (instr >> 21) & 0x1F,
                  (instr >> 16) & 0x1F, (instr >> 11) & 0x1F);
        break;
      case FORMAT_RRX:
        if ((opcode & 1) == 0) {
