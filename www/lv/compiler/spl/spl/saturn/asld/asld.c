/*
 * asld.c -- ECO32 assembler and linking loader
 */
...
#define STATUS_GLOBREF	3	/* local entry refers to a global one */
...
char codeName[] = "/tmp/asld_code_XXXXXX";
char dataName[] = "/tmp/asld_data_XXXXXX";
char *outName = NULL;
char *mapName = NULL;
char *inName = NULL;
...
typedef struct fixup {
  ...
  int reloc;			/* segment to which this ref is relative */
...
void makeSegments(void) {
  ...
  if (!segStartDefined[SEG_DATA]) {
    segStart[SEG_DATA] = segStart[SEG_CODE] + segPtr[SEG_CODE];
...
void doFixup(Fixup *f) {
  ...
  switch (f->method) {
    case METHOD_H16:
      value = f->value + segStart[f->reloc];
  ...
  if (debugFixup) {
    printf("DEBUG: fixup (s:%s, o:%08X, m:%s, v:%08X), %08X --> %08X\n",
           segName[f->segment], f->offset, methodName[f->method], f->value,
           value, final);
  }
}


void fixupSegments(void) {
  Fixup *f;
...
static void printToMap(Symbol *s, FILE *mapFile) {
  if (mapFile == NULL) {
    return;
  }
  ...
  switch (s->status) {
    ...
    case STATUS_DEFINED:
      ...
      break;
    case STATUS_GLOBREF:
      fprintf(mapFile, "%-15s", "GLOBAL");
      break;
    default:
      error("illegal symbol status in printToMap()");
  }
...
static void linkSymbol(Symbol *s, FILE *mapFile) {
  Fixup *f;

  printToMap(s, mapFile);
  ...
  if (s->status == STATUS_GLOBREF) {
    ...
    while (s->fixups != NULL) {
      ...
      if (debugFixup) {
        printf("DEBUG: link '%s' (s:%s, o:%08X, m:%s, v:%08X --> %08X)\n",
               s->name, segName[f->segment], f->offset, methodName[f->method],
...
static void recursiveLinkAndFree(Symbol *s, FILE *mapFile) {
  if (s == NULL) {
    return;
  }
  recursiveLinkAndFree(s->left, mapFile);
  linkSymbol(s, mapFile);
  freeMemory(s->name);
  freeMemory(s);
  recursiveLinkAndFree(s->right, mapFile);
}


void linkAndFree(int whichTable) {
  if (whichTable == GLOBAL_TABLE) {
    recursiveLinkAndFree(globalTable, mapFile);
    globalTable = NULL;
  } else {
    recursiveLinkAndFree(localTable, NULL);
    localTable = NULL;
  }
...
void formatRH(unsigned int code) {
  ...
  v = parseExpression();
  if (v.sym == NULL) {
    emitHalf(code << 10 | reg << 5);
    emitHalf(v.con);
  } else {
    addFixup(v.sym, currSeg, segPtr[currSeg] + 2, METHOD_L16, v.con);
    emitHalf(code << 10 | reg << 5);
    emitHalf(0);
  }
}
...
void formatRHh(unsigned int code) {
  ...
  if (v.sym == NULL) {
    ...
    addFixup(v.sym, currSeg, segPtr[currSeg] + 2, METHOD_H16, v.con);
    emitHalf(code << 10 | reg << 5);
    emitHalf(0);
  }
}
  ...
  if (allowSyn) {
    if (v.sym == NULL) {
      if ((v.con & 0xFFFF0000) == 0) {
        /* code: op dst,src,con */
        emitHalf(code << 10 | dst << 5 | src);
        emitHalf(v.con);
      } else {
        /* code: ldhi $1,con; or $1,$1,con; add $1,$1,src; op dst,$1,0 */
        emitHalf(OP_LDHI << 10 | 1 << 5);
        emitHalf(v.con >> 16);
        emitHalf((OP_OR + 1) << 10 | 1 << 5 | 1);
        emitHalf(v.con);
        emitHalf(OP_ADD << 10 | 1 << 5 | 1);
        emitHalf(src << 11);
        emitHalf(code << 10 | dst << 5 | 1);
        emitHalf(0);
      }
    } else {
      /* code: ldhi $1,con; or $1,$1,con; add $1,$1,src; op dst,$1,0 */
...
void formatRRB(unsigned int code) {
  ...
  if (v.sym == NULL) {
    ...
    addFixup(v.sym, currSeg, segPtr[currSeg] + 2,
             METHOD_R16, v.con - ((signed) segPtr[currSeg] + 4));
    /* note: division by 4 must be deferred until symbol has been added */
    immed = 0;
  }
...
void formatJ(unsigned int code) {
  ...
  if (token == TOK_REGISTER) {
    ...
    if (v.sym == NULL) {
      ...
      addFixup(v.sym, currSeg, segPtr[currSeg],
               METHOD_R26, v.con - ((signed) segPtr[currSeg] + 4));
      /* note: division by 4 must be deferred until symbol has been added */
      immed = 0;
    }
    emitHalf(code << 10);
...
void writeTrailer(void) {
  ...
  if (!withHeader) {
    /* no header means plain binary: must fill BSS with zeroes */
    for (i = 0; i < segPtr[SEG_BSS]; i++) {
      fputc(0, outFile);
    }
  }
}
...
int readNumber(char *str, int *np) {
  int base;
  int value;
  int digit;
...
int main(int argc, char *argv[]) {
  ...
  do {
    ...
    linkAndFree(LOCAL_TABLE);
  } while (++i < argc);
  fprintf(stderr, "Linking modules...\n");
  linkAndFree(GLOBAL_TABLE);
